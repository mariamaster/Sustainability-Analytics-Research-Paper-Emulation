---
title: "ASN6.2. Paper Replication: Make Figure 1"
...
![](Fig1.JPG){fig-align="center" width="500"}

### Part 1: Prepare data and packages

Step 1: Use the **library** function to declare that you will use the **tidyverse** and **lubridate** package in this session. **(2 points)**
...
Step 2: Re-run the script that you wrote in ASN6.1.
...
This step uploads the data into the Quarto environment, stacks them into one data frame, and creates a new column called NOx_emit that replaces \_\_NA\_\_ values in the column \_\_NOx..tons\_\_ with the value \_\_0\_\_.
...
#### Answer: The average total \_\_daily\_\_(annual/monthly/daily) NOx emissions from regulated states
...
```{r}
df_nbp3 <- df_nbp2 %>%
  mutate(NOx_daily = ifelse(Month == 1 | Month == 3 | Month == 5 | Month == 7 | Month == 8| Month == 10 | Month == 12, NOx_emit/31, NA ))
```
...
```{r}
df_nbp4 <- df_nbp3 %>%
  mutate(NOx_daily = ifelse(Month == 4 | Month == 6 | Month == 9 | Month == 11, NOx_emit/30, NOx_daily))
```
...
```{r}
df_nbp5 <- df_nbp4 %>%
  mutate(NOx_daily = ifelse(Month == 2, NOx_emit/28, NOx_daily))
```
...
Step 1: Use the **ggplot** function to create a line graph. The graph should have two lines, each showing the annual average NOx emissions from each state before and after the implementation of the NOx Budget Program. The graph should look like or similar the one below. **(6 points)**
...
```{r}
library(ggplot2)
ggplot(df_nbp9, aes(x = doy, y = NOx_th, color = phase))+geom_line()
```
...
Hint: You learned this in the podcast, or you can find the answer in the Introduction section of [this paper](https://pubs.aeaweb.org/doi/pdfplus/10.1257/aer.20131002).

#### Answer: The NOx Budget program was in effect during the middle of the year.
...
#### Answer: This graph is taking into consideration the average of the daily NOx emit.
